...
- [X] 자판기가 보유한 잔돈 기능 처리
    ...
    - [X] 잔돈을 주어진 단위에 따라 무작위로 개수 생성
      ...
      - [X] Coin Enum 사용
    - [X] 자판기가 보유한 동전에 대한 출력 형식에 맞게 출력
- [X] 상품명, 가격, 수량을 받은 입력 처리
    - [X] (예외) 입력받을 때 ;로 구분자 처리가 잘 되었는지
    - [X] (예외) 입력받을 때 [] 로 잘 묶여있는지
        - [X] []가 한쌍이 아닌 경우
        - [X] 대괄호가 맨 처음과 맨 마지막을 묶는 것이 아닌 경우
    - [X] 가격, 수량에 대하여 값을 저장하는 처리
- [X] 투입 금액에 대한 입력
  - [X] (예외) 금액 입력이 숫자가 아닌 경우가 올 때
  - [X] (예외) 10원 단위로 나누어 떨어지지 않을 경우
- [X] 상품 구매 프로세스 진행
    - [X] 프로세스 종료 조건 생각
        - [X] 수량이 더 이상 없을 때
        - [X] 투입 금액이 상품가격중 최소 가격보다 작을 때
        - [X] 상품이 전부 매진 될 경우
        - [X] (예외) 상품 입력시 없는 상품을 집을 경우
- [X] 종료 후 남은 투입 금액에 대한 잔돈 처리
